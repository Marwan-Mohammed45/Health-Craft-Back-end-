...
export const patientSignup = asyncHandler(async (req, res) => {
  ...
  // إنشاء OTP للتحقق من الإيميل
  const otp = Math.floor(100000 + Math.random() * 900000).toString();
  const otpExpire = Date.now() + 10 * 60 * 1000; // 10 دقائق
  ...
  res.status(201).json({
    success: true,
    message: "Patient registered. Please check your email for OTP.",
  });
});
...
export const patientSignin = asyncHandler(async (req, res) => {
  ...
  const token = createToken({ id: patient._id, role: "patient" });
